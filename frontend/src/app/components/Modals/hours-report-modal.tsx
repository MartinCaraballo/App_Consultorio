...
const HoursReportModal = ({isOpen, onClose}: { isOpen: boolean, onClose: () => void }) => {
    ...
    async function fetchUserReserves(startDate: string, endDate: string) {
        ...
        axiosInstance
            .get<ReserveDTO[]>(
                `/reserve/get-user-reserves?startDate=${startDate}&endDate=${endDate}`
            )
    ...
    async function fetchUserMonthlyCost(startDate: string, endDate: string) {
        ...
        axiosInstance
            .get<number>(
                `/user/monthly-cost-between-dates?startDate=${startDate}&endDate=${endDate}`
            )
    ...
    React.useEffect(() => {
        fetchData(formatDate(selectedStartDate), formatDate(selectedEndDate))
    }, []);
    ...
    return (
        <Dialog open={isOpen} onClose={onClose}
            ...
            <div className="bg-white p-6 rounded-2xl shadow-xl w-full md:max-w-screen-md lg:max-w-screen-xl h-2/3 overflow-y-auto">
                {loading && (
                ...
                <div className="flex flex-row justify-center">
                    <div
                        className="flex flex-col justify-self-center mt-2 w-full px-8 lg:w-2/3 lg:px-0 space-y-4">
                        <ul className="text-lg xl:text-xl space-y-1">
                            {userReserves.map((reserve, index) => {
                                ...
                                return (
                                    <li
                                        key={index}
                                        className="flex items-center justify-around border-b rounded-lg font-bold bg-gray-700 text-center text-white flex-col sm:flex-row h-36 sm:h-12"
                                    >
                                    <span>
